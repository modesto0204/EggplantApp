package com.example.eggplant.component
...
import com.example.eggplant.R
import com.example.eggplant.patternmodel.Dot
import com.example.eggplant.patternmodel.DotView
...
):LinearLayout(context,attrs,defStyleAttr)
{
    ...
    init {
        context.theme.obtainStyledAttributes(
            ...
            try {
                ...
                attrSuccessDotColor = getColor(R.styleable.PatternLockView_patternLock_successDotColor,Color.BLUE)
                attrSuccessLineColor = getColor(R.styleable.PatternLockView_patternLock_successLineColor,Color.BLUE)
                attrErrorDotColor = getColor(R.styleable.PatternLockView_patternLock_errorDotColor,Color.RED)
                attrErrorLineColor = getColor(R.styleable.PatternLockView_patternLock_errorLineColor,Color.RED)
                    ...
                    if (markedDotList.size != 0 && markedDotList.size >= minCount) {
                        when (stageState) {
                            ...
                            PatternViewStageState.SECOND -> {
                                ...
                                state = if (stagePasswords[PatternViewStageState.FIRST]
                                    != stagePasswords[PatternViewStageState.SECOND]
                                ) {
                                    PatternViewState.Error(attrErrorDotColor, attrErrorLineColor)
                                } else {
    ...
    private fun createColumn(
        view:LinearLayout,nodeKey:String
    ){
    ...
    private fun isTouchedDot(pointX: Float, pointY: Float): Boolean {
        ...
        markedDotList.takeIf { it.size != 0 }?.last().let { lastTouchedDot->
            val rowIndex = (lastTouchedDot!!.rowIndex + touchedDot.rowIndex
                    ) /2
    ...
    private fun isDotSelected(dot: Dot) = markedDotList.firstOrNull{markedDot->
        markedDot.rowIndex == dot.rowIndex
                && markedDot.columnIndex == dot.columnIndex
